...
To Stepper motor λειτουργεί διαφορετικά από τα άλλα μοτεράκια. Η κίνηση δεν γίνεται συνεχόμενα όπως π.χ. στο DC motor, αλλά περιστρέφεται με βήματα. Από τα τεχνικά χαρακτηριστικά του Stepper Motor 28BYJ-48 που αναφέρουμε παραπάνω, για να πραγματοποιήση μια πλήρη περιστροφή απαιτούνται 32 βήματα. Αυτό προκύπτει με βάση τους παρακάτω υπολογισμούς. 

Για κάθε οκτώ φάσεις των πηνίων του stepper, τα οποία ενεργοποιούνται από το Arduino, διανύεται ένα τόξο 11.25 μοίρες. Εάν διαιρέσουμε τις 360 μοίρες που έχει ο κύκλος με τις μοίρες του τόξου, έχουμε τον αριθμό των βημάτων που κάνει το μοτέρ δηλαδή 32 βήματα.
(360 μοίρες/11.25 μοίρες = 32 steps). 

Στην πραγματικότητα το stepper με βάση μια διάταξη από γρανάζια στο εσωτερικό του, τα οποία μας δείνουν μια αναλογία 1 προς 64 (1/64), πάλι όπως αναφέρεται στις προδιαγραφές, για κάθε περιστροφή που βλέπουμε εμείς στον εξωτερικό άξονα, το stepper στο εσωτερικό του προγματοποιεί 64 περιστροφές. Γι'αυτό εάν θέλουμε να προγραμματίσουμε το stepper να κάνει μία περιστροφή, θα πρέπει στην μεταβλητή μας να δόσουμε την τιμή (32 steps Χ 64 περιστροφές) = 2048 περιστροφές. Για δύο περιστροφές την τιμή 4096 για μισή περιστροφή την τιμή 1024 κ.ο.κ. Με ευτόν τον τρόπο μπορούμε να μετακινήσουμε με μεγάλη ακρίβεια την ρομποτική μας εφαρμογή.   
...
# Προγραμματισμός ενός Stepper motor 28BYJ-48
    ...
    int val = 1024;                         // Variable - Ορίζουμε την μεταβλητή "val" την κίνηση το stepper motor, 
...
*****************************************************************************************************************
 ...
 # Το πρόγραμμα με δύο Stepper motors 28BYJ-48
    ...
    void setup(){                           // Η συνάρτηση SETUP εκτελείται μία φορά.
        stepper.setSpeed(600);              // Η συνάρτηση SetSpeed" (στορφές ανα λεπτό) δηλώνει την ταχύτητα περιστοφής του motor.
    ...
    void loop() {                           // Η συνάρτηση LOOP
       stepper.step(val);                   // Η συνάρτηση stepper.step δηλώνει κίνηση προς μία κατευθυνση του step motor.
       delay(1000);                         // Καθυστέρηση 1 δευτερόλεπτο.
       stepper1.step(val1);                 // Η συνάρτηση stepper1.step δηλώνει κίνηση προς μία κατευθυνση του step motor.
       delay(1000);                         // Καθυστέρηση 1 δευτερόλεπτο.
       stepper.step(-val);                  // Η συνάρτηση stepper.step - δηλώνει κίνηση προς την αντίθετη κατεύθυνση του step motor.
       delay(1000);                         // Καθυστέρηση 1 δευτερόλεπτο.
